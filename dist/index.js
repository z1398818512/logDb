...
/* harmony import */ var _wss__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(84);
...
class serve {
  ...
  initIo() {
    const socket = (0,_wss__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .Z)(this.serveUrl, {
        ...
        wss.onmessage = (res) => {
            ...
            try {
                ...
                if (type === 'connection') {
                    ...
                    console.log(`%c  已连接logDB后台，打开链接： %c https://oivoee-admin.site.laf.run/?roomId=${roomId}`, `color: #51ef36; background: #1d1c1c; padding:2px 5px;`,``);
                } else {
                    fun = onMap.get(type);
                }
                if (fun) {
...
class loggerDb extends Dexie$1 {
    ...
    /* 启动在线可视化页 */
    openOnline(serveUrl) {
        this.socket = new serve/* default */.Z({ db: this, room: this.room, serveUrl: serveUrl }) // 启动在线分析
    }
